...
- The Replit static host does NOT auto-resolve `/route` → `/route/index.html`. A single catch-all `/* → /index.html` intercepts EVERY clean URL and serves the homepage HTML on direct/organic access (only `/route/index.html` worked). This is the bug behind "body links 404 on direct access."
- **FIX (current `artifact.toml`): explicit per-route rewrites — one `[[services.production.rewrites]] from="/<route>" to="/<route>/index.html"` block for every prerendered dir — then the SPA catch-all `/* → /index.html` LAST.** All flight-school routes are flat (single-segment), 86 of them.
- **Generate the rewrites from `dist/public` output, never hand-maintain.** After adding/removing routes, rebuild then regenerate: `find dist/public -mindepth 2 -maxdepth 2 -name index.html`. Edit `artifact.toml` only via `verifyAndReplaceArtifactToml` (temp-file workflow), not directly.
...
- `prerender.mjs` reads `dist/public/index.html` as a template (must contain `<!--app-head-->` and `<!--app-html-->` placeholders), renders each route, writes `dist/public/<route>/index.html`. The static host serves real asset files first, but clean URLs need the explicit per-route rewrites above (it will NOT auto-map `/route`→`/route/index.html`).
