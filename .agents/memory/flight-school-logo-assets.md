...
- **Favicons** are generated via ImageMagick `magick` (PIL/sharp unavailable) from the **AFS monogram** of `afs-logo-new.png` — crop `1001x250+56+159` then `-trim` (the big "AFS" letters + orange swoosh, EXCLUDING the "ACCELERATED FLIGHT SCHOOL" banner), then `-resize 452x452 -background white -gravity center -extent 512x512` to a square master, then downscale. Active set in `public/`: `favicon.ico` (bundles 16/32/48), `favicon-16.png`, `favicon-32.png`, `apple-touch-icon.png` (180); linked from `index.html`.
- **Gotcha (fixed once):** an earlier favicon set was a broken zoomed/auto-crop that captured only a meaningless orange/blue sliver (Google rendered it as an orange square). Do NOT auto-trim the full wordmark blindly or feed the whole logo — fit the AFS monogram into a square on WHITE (blue letters need contrast; navy bg blends). White bg is also what Google's results card expects. **Republish required** for favicon changes (static deploy); Google re-crawls the favicon on its own slow schedule (days–weeks), independent of sitemap submission.
